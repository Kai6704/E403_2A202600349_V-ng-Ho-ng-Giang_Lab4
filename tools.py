...
HOTELS_DB = {
    ...
    "Phú Quốc": [
        {"name": "Vinpearl Resort", "stars": 5, "price_per_night": 2_500_000, "area": "Bãi Dài", "rating": 4.4},
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    if not filtered:
        return f"Không tìm thấy khách sạn tại {city} với giá dưới {f'{max_price_per_night:,}'.replace(',', '.')}đ/đêm. Hãy thử tăng ngân sách."
        
    sorted_hotels = sorted(filtered, key=lambda x: x["rating"], reverse=True)
    
    lines = [f"Khách sạn tại {city} (ngân sách dưới {f'{max_price_per_night:,}'.replace(',', '.')}đ/đêm):"]
    for h in sorted_hotels:
        price_str = f"{h['price_per_night']:,}".replace(",", ".") + "đ"
        lines.append(f"- {h['name']} ({h['stars']} sao) | Khu: {h['area']} | Rating: {h['rating']} | Giá: {price_str}")
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    """
    try:
        expense_items = [item for item in expenses.split(',') if item.strip()]
        parsed_expenses = {}
        total_spent = 0
        ...
        for item in expense_items:
            ...
            parsed_expenses[name.strip().replace('_', ' ').capitalize()] = amount
            total_spent += amount
    except ValueError:
        return "Lỗi: Định dạng chi phí không hợp lệ. Vui lòng sử dụng định dạng 'tên_khoản:số_tiền,tên_khoản:số_tiền'."
        
    remaining = total_budget - total_spent
    
    lines = ["Bảng chi phí:"]
    for name, amount in parsed_expenses.items():
        lines.append(f"- {name}: {f'{amount:,}'.replace(',', '.')}đ")
    ...
    if remaining < 0:
        lines.append(f"Vượt ngân sách {f'{-remaining:,}'.replace(',', '.')}đ! Cần điều chỉnh.")
